...
```
执行顺序：先解除映射 → 再从注册表注销 → 最后删除共享内存文件。
- 正常销毁时主动从注册表移除，后续即使进程崩溃，也不会重复清理已经删掉的 shm；
- 先注销再删除，避免中间窗口信号触发，对已经删除的文件执行无意义的 `shm_unlink`（虽然不会崩溃，但属于冗余操作）。
...
5.  先解除映射，再从注册表注销名称，最后 `shm_unlink` 删除文件。
